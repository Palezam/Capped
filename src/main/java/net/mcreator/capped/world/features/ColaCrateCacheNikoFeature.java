...
package net.mcreator.capped.world.features;
...
public class ColaCrateCacheNikoFeature extends Feature<NoneFeatureConfiguration> {
	public static final Predicate<BiomeSelectionContext> GENERATE_BIOMES = BiomeSelectors.all();
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(Level.OVERWORLD);
	private StructureTemplate template = null;

	public ColaCrateCacheNikoFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("capped", "crate_cache_niko"));
		if (template == null)
			return false;
